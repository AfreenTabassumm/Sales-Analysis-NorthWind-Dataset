...
-- Q6 How frequently do different customer segments place orders?
	 ...
	 datediff(lead(orderdate) over( partition by CustomerID),orderdate) as days
	 from orders;
...
select employeeid , title , TitleOfCourtesy, count(*) as no_of_employee
 from employee
group by 1,2,3;
...
select * from order_details;
    ...
    alter	 table order_details add column SalesPrice decimal;
    
    update order_details set salesprice = UnitPrice * Quantity *(1-Discount)
